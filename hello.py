...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'HowMuchWoodWouldAWoodChuckChuckIfAWoodChuckCouldChuckWood?'
...
def index():
    ...
    if form.validate_on_submit():
        ...
        # Check if email is valid utoronto email
        if 'utoronto' not in form.email.data:
            session['email_warning'] = True
        else:
            session['email_warning'] = False 
